import { h } from "vue";
import { isDef, isLinkHttp } from "@vuepress/helper/client";

//#region src/client/components/IconBase.ts
const IconBase = ({ name = "", color = "currentColor", ariaLabel }, { attrs, slots }) => h("svg", {
...
//#endregion
//#region src/client/components/fullscreenIcons.ts
...
const generateIndexFromHash = (content, total) => cyrb53(content) % total;
...
const normalizePath = (path) => decodeURI(path).replace(HASH_REGEXP, "").replace(/\/index\.html$/iu, "/").replace(/\/(README|index)\.md$/iu, "/").replace(/\.(?:html|md)$/iu, "");
/**
...
//#endregion
//#region src/client/utils/window.ts
const getSize = () => {
...
const openPopupWindow = (link, target = "_blank", features = ["resizable", "status"]) => {
	const { width, height, left, top } = getSize();
	globalThis.open(link, target, `width=${width},height=${height},left=${left},top=${top},${features.join(",")}`)?.focus();
};

//#endregion
export { BitbucketIcon, CancelFullScreenIcon, EnterFullScreenIcon, GitHubIcon, GitLabIcon, GiteeIcon, IconBase, RepoIcon, SourceIcon, cyrb53, generateIndexFromHash, getHash, getSize, isActiveLink, normalizePath, openPopupWindow, resolveRepoLink, resolveRepoType };
