        ...
        if (matchedKeys.length > 0) {
            ...
            return {
                ...
                isLocked: matchedKeys.some((key) => (localTokenConfig.value[key]
                    ? config[key].tokens.every((token) => !isTokenMatched(token, localTokenConfig.value[key]))
                    : true) &&
                    (sessionTokenConfig.value[key]
                        ? config[key].tokens.every((token) => !isTokenMatched(token, sessionTokenConfig.value[key]))
                        : true)),
