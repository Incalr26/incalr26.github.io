...
const handleHTML = (html) => html.replaceAll(String.raw`<br \/>`, "<br>").replaceAll(/<((\S+)[^<]*?)\s+\/>/gu, "<$1></$2>");
const getHtmlTemplate = (html) => `<div id="app">\n${handleHTML(html)}\n</div>`;
const getReactTemplate = (code) => `${code.replace("export default ", "const $reactApp = ").replace(/App\.__style__(\s*)=(\s*)`([\s\S]*)?`/u, "")};\nReactDOM.createRoot(document.getElementById("app")).render(React.createElement($reactApp))`;
...
const getReactCode = (code, config) => {
	...
	return {
		...
		css: code.css[0] ?? code.js[0]?.replace(/App\.__style__(?:\s*)=(?:\s*)`([\s\S]*)?`/u, "$1").trim() ?? "",
		isLegal: code.isLegal,
		jsLib: [
			codeConfig.react,
			codeConfig.reactDOM,
