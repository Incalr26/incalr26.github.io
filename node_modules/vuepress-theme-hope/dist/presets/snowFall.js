...
//#region src/presets/snowFall.ts
...
const setupSnowFall = ({ speed = 1, image = DEFAULT_SNOWFLAKE_IMAGE, count = 10, minSize = 5, maxSize = 10 } = {}) => {
	onMounted(() => {
		...
		snowflakeImage.addEventListener("load", () => {
			const snowflakes = [];
			for (let i = 0; i < count; i++) snowflakes.push(new Snowflake(snowflakeImage, context, {
				maxSize,
				minSize,
				speed
			}));
